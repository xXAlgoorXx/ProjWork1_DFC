...
image = preprocess(Image.open("models/temp/images.jpeg")
...
typeModel = model.dtype
tokenEmb = model.token_embedding
posEmb = model.positional_embedding
TextTransformer = model.transformer
layerNorm = model.ln_final
textProj = model.text_projection

# print("Label probs:", probs)  # prints: [[0.9927937  0.00421068 0.00299572]]


def encode_text_hailo(text):
    x = tokenEmb(text).type(typeModel)  # [batch_size, n_ctx, d_model]
    x = x + posEmb.type(typeModel)
    x = x.permute(1, 0, 2)  # NLD -> LND
    x = TextTransformer(x)
    x = x.permute(1, 0, 2)  # LND -> NLD
    x = layerNorm(x).type(typeModel)

    # x.shape = [batch_size, n_ctx, transformer.width]
    # take features from the eot embedding (eot_token is the highest number in each sequence)
    x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ textProj

    return x


def dummy_text_hailo(text):
    x = tokenEmb(text).type(typeModel)  # [batch_size, n_ctx, d_model]
    x = x + posEmb.type(typeModel)
    x = x.permute(1, 0, 2)  # NLD -> LND
...
# Create dummy input
dummy_input = dummy_text_hailo(text)

# Export the model to ONNX format
modelPath = f"models/temp/CLIP_{model_name.replace('/', '_')}_text.onnx"
TextTransformer.eval()  # Set model to evaluation mode
torch.onnx.export(
    TextTransformer,                    # Model being run
...
print(f"ONNX saved at {modelPath}")
...
print(f"Take model from {simplified_path}")
